...
def create_tables():
    ...
    # Default super_admin user (password: SuperAdmin1!)
    sa_hash = hashlib.sha256("SuperAdmin1!".encode()).hexdigest()
    cur.execute("""INSERT INTO users (username, password_hash, role, full_name, email, department, must_change_password)
                   VALUES ('super_admin', %s, 'SUPER_ADMIN', 'Super Administrator', 'superadmin@opticlass.local', 'System', false)
                   ON CONFLICT (username) DO NOTHING""", (sa_hash,))
    conn.commit()
    cur.close()
    conn.close()
    _init_log_file()
...
def suggest_weekly_schedule(body: WeeklyScheduleRequest, token_data: dict = Depends(verify_token)):
    ...
    for title, description, sort_fn in strategies:
        ...
        for course in ordered:
            ...
            # Fallback: if no valid slot found with full constraints, relax semester/COMMON conflicts
            # Hard constraints kept: instructor availability + no instructor double-booking + free classroom
            if best is None:
            ...
            if best:
                ...
                if lab_hrs > 0:
                    ...
                    for search_day in (other_days + [b_day]):
                        ...
                        if lab_best:
                            break
                    # Lab fallback: ignore semester conflicts, keep instructor availability
                    if lab_best is None:
                        all_days_lab = sorted(
                            XAI_SCHEDULE_DAYS,
