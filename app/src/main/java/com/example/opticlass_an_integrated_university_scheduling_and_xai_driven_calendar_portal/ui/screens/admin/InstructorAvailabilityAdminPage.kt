    ...
    Column(modifier = Modifier.fillMaxSize().padding(16.dp)) {
        if (selectedInstructor == null) {
            ...
            if (availability != null) {
                ...
                // Müsaitlik durumunu SnapshotStateMap olarak oluşturuyoruz
                val slotsState = remember(availability.instructorName) {
                    val map = mutableStateMapOf<String, Set<String>>()
                    availability.slots.forEach { (day, slotList) ->
                        map[day] = slotList.toSet()
                    }
                    map
                }
